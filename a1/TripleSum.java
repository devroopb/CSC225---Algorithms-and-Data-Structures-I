...
public class TripleSum{
	...
	*/
	public static boolean TripleSum225(int[] A){
		...
		int n = A.length;
		int [] Devroop225 = new int [10000000];
		boolean result = false;
		int meaningfulVariable = 0;
		int findTheLastNumber;
		...
		for(int c = 0; c < n; c++){
			if(A[c] <= 225){
				Devroop225[meaningfulVariable] = A[c];
				meaningfulVariable++;
			}
		}
		...
		for(int i = 0; i < 1500; i++){
			for(int j = 0; j < 1500; j++){
				if( j == i){
					continue;
				}else if(Devroop225[i] + Devroop225[j] <= 225){
					findTheLastNumber = 225 - (Devroop225[i] + Devroop225[j]);
					for(int k = 0; k < 1500; k++){
						if((k == i)||(k == j)){
							continue;
						}else if(Devroop225[k] == findTheLastNumber){
							result = true;
						}
					}
				}
